    ...
    <main className="min-h-screen bg-[#0b0b0b] text-white">
      <header className="border-b border-white/10 bg-[#0b0b0b]/95 backdrop-blur">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-5 py-4 sm:px-8 sm:py-5">
          <div className="flex items-center gap-3">
            <div className="flex h-9 w-9 items-center justify-center rounded-full bg-[#e30613] shadow-[0_0_24px_rgba(227,6,19,0.22)]"><span className="h-3 w-3 rounded-full border-2 border-white" /></div>
            <div><p className="text-lg font-bold tracking-tight">CRM Voice Bot</p><p className="mt-0.5 text-xs text-[#8e8e93]">Straumann Projektdemo</p></div>
          </div>
          <nav className="flex gap-2">
            <a href="#besuche" className="rounded-full border border-white/15 bg-[#181818] px-3 py-2 text-xs font-semibold hover:border-[#e30613]/60 hover:bg-[#222] sm:px-4 sm:text-sm">Meine Anrufe</a>
            <a href="#feedback" className="hidden rounded-full border border-white/15 bg-[#181818] px-4 py-2 text-sm font-medium hover:border-[#e30613]/60 hover:bg-[#222] sm:inline-flex">Feedback</a>
          </nav>
        </div>
      </header>
      ...
      <section className="mx-auto grid max-w-6xl gap-10 px-5 py-12 sm:px-8 sm:py-16 lg:grid-cols-[1fr_440px] lg:items-center">
        <div>
          ...
          <h1 className="max-w-3xl text-4xl font-bold leading-tight tracking-tight sm:text-6xl">Besuchsdokumentation einfach per Sprache.</h1>
          ...
          <div className="mt-8 grid max-w-2xl gap-3 sm:grid-cols-3">
            {[
              ...
              <div key={nr} className="rounded-2xl border border-white/10 bg-[#151515] p-4">
                <p className="text-xs font-bold uppercase tracking-[0.16em] text-[#e30613]">{nr}</p><p className="mt-2 text-sm font-semibold">{titel}</p><p className="mt-1 text-xs leading-5 text-[#8e8e93]">{text}</p>
              </div>
            ))}
          </div>
          <div className="mt-7 flex flex-wrap gap-3">
            <a href="#demo" className="rounded-2xl bg-[#e30613] px-6 py-4 font-bold shadow-[0_12px_36px_rgba(227,6,19,0.20)] hover:bg-[#c9000b]">Voice Bot testen</a>
            <a href="#feedback" className="rounded-2xl border border-white/15 px-5 py-4 text-sm font-semibold text-[#c7c7cc] hover:border-[#e30613]/60 hover:text-white">Danach Feedback geben</a>
          </div>
          <p className="mt-4 text-sm text-[#666]">Demo ausschließlich mit Testdaten</p>
        </div>

        <form id="demo" onSubmit={startVisit} className="rounded-[28px] border border-white/10 bg-[#181818] p-5 shadow-2xl sm:p-6">
          <div className="rounded-3xl bg-white p-6 text-[#111] sm:p-8">
            ...
            <h2 className="text-center text-2xl font-bold">{busy ? "Läuft …" : "Besuch starten"}</h2>
            ...
            <div className="mt-4 rounded-xl border border-[#e30613]/15 bg-[#fff5f5] px-4 py-3 text-center text-xs font-medium leading-5 text-[#6f3034]">
              Nach dem Gespräch erscheint der dokumentierte Besuch weiter unten unter{" "}
              <button type="button" onClick={zuBesuchenScrollen} className="font-bold text-[#e30613] underline decoration-[#e30613]/30 underline-offset-2">Meine Anrufe</button>.
            </div>
            <label htmlFor="phone" className="mt-6 block text-sm font-semibold">Telefonnummer</label>
            <input id="phone" type="tel" inputMode="tel" autoComplete="tel" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="+49 …" disabled={busy} className="mt-2 w-full rounded-2xl border border-[#d7d7d7] bg-white px-4 py-4 text-base outline-none focus:border-[#e30613] disabled:bg-[#eee]" />
          ...
          <div className="mt-4 flex items-start gap-4 rounded-2xl bg-[#101010] p-4">
            <span className={`mt-1 h-2.5 w-2.5 shrink-0 rounded-full ${busy ? "bg-[#ff9f0a]" : callState === "error" ? "bg-[#e30613]" : "bg-[#35c759]"}`} />
      ...
      {visits.length > 0 && (
        <button type="button" onClick={zuBesuchenScrollen} className="fixed bottom-4 left-4 right-4 z-40 flex items-center justify-between gap-3 rounded-2xl border border-[#e30613]/30 bg-[#171313]/95 px-4 py-3 text-left shadow-2xl backdrop-blur sm:hidden">
          ...
          <span className="shrink-0 rounded-full bg-[#e30613] px-3 py-2 text-xs font-bold">Ansehen</span>
        </button>
      )}

      <section id="besuche" ref={visitsSectionRef} className="scroll-mt-4 border-y border-white/10 bg-[#101010]">
        <div className="mx-auto max-w-6xl px-5 py-16 sm:px-8 sm:py-20">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div><p className="text-sm font-semibold uppercase tracking-[0.18em] text-[#e30613]">Meine Anrufe</p><h2 className="mt-3 text-3xl font-bold sm:text-4xl">Erfasste Besuche prüfen und bearbeiten.</h2></div>
            {visits.some(istUnvollstaendig) && <button type="button" onClick={() => visits.filter(istUnvollstaendig).forEach(aktualisierenVisit)} disabled={refreshingId !== null} className="rounded-full border border-white/15 bg-[#181818] px-4 py-2 text-sm font-medium disabled:opacity-45">Alle wartenden aktualisieren</button>}
          </div>
            ...
            <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {visits.map((visit) => {
                ...
                return <div key={visit.visit_id} className="rounded-3xl border border-white/10 bg-[#181818] p-5 hover:bg-[#1f1f1f]">
                  ...
                  {offen && <button type="button" onClick={() => aktualisierenVisit(visit)} disabled={refreshingId === visit.visit_id} className="mt-4 w-full rounded-xl border border-white/15 bg-[#101010] px-4 py-2 text-xs font-semibold disabled:opacity-45">{refreshingId === visit.visit_id ? "Wird aktualisiert …" : "Aktualisieren"}</button>}
                </div>;
              })}
            </div>
          )}
          ...
          <div className="w-full max-w-2xl rounded-[28px] border border-white/10 bg-[#0b0b0b]">
            ...
            <div className="max-h-[70vh] overflow-y-auto px-6 py-6">
              {editing && editVisit ? (
                ...
                <div>
                  {istUnvollstaendig(selected) && <button type="button" onClick={() => aktualisierenVisit(selected)} className="mb-5 w-full rounded-2xl border border-white/15 bg-[#181818] px-5 py-3 font-semibold">Auswertung aktualisieren</button>}
                  <button type="button" onClick={bearbeitenStarten} className="w-full rounded-2xl border border-white/15 bg-[#181818] px-5 py-3 font-semibold">Bearbeiten</button>
                  ...
                  {(selected.verkaufsprozess?.length ?? 0) > 0 && <div className="mt-6"><p className="mb-3 text-sm font-semibold uppercase tracking-wider text-[#8e8e93]">Verkaufsprozess</p>{selected.verkaufsprozess?.map((vp, i) => <div key={i} className="mb-3 rounded-2xl border border-white/10 bg-[#151515] p-4"><Anzeige label="Ziel" wert={vp.ziel} /><Anzeige label="Lösung" wert={vp.loesung} /><Anzeige label="Brand" wert={vp.brand} /><Anzeige label="Fokus" wert={vp.fokus} /></div>)}</div>}
      ...
      <section className="mx-auto grid max-w-6xl gap-10 px-5 py-16 sm:px-8 sm:py-20 lg:grid-cols-2 lg:items-center">
        <div><p className="text-sm font-semibold uppercase tracking-[0.18em] text-[#e30613]">Funktionsumfang</p><h2 className="mt-3 text-3xl font-bold sm:text-4xl">Ein Ablauf, verschiedene Eingabemöglichkeiten.</h2><p className="mt-5 max-w-xl leading-7 text-[#8e8e93]">Die Web-Demo zeigt den Voice Bot. Die Diktierfunktion und die vollständige Nachbearbeitung werden zusätzlich in der mobilen App demonstriert.</p></div>
        <div className="space-y-3">{features.map((feature) => <div key={feature} className="flex items-center gap-4 rounded-2xl border border-white/10 bg-[#181818] p-4"><span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-[#e30613] font-bold">✓</span><span>{feature}</span></div>)}</div>
      </section>
      ...
      <section id="feedback" className="border-t border-white/10 bg-[#101010] px-5 py-16 sm:px-8 sm:py-20">
        <div className="mx-auto max-w-3xl rounded-[28px] border border-white/10 bg-[#181818] p-6 sm:p-10">
          <p className="text-sm font-semibold uppercase tracking-[0.18em] text-[#e30613]">Feedback & Ideen</p><h2 className="mt-3 text-3xl font-bold">Was würde dir im Alltag helfen?</h2><p className="mt-3 leading-7 text-[#9b9b9b]">Teile Verbesserungsvorschläge, fehlende Funktionen oder konkrete Anwendungsfälle.</p>
